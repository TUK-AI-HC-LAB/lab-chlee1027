...
> **주의 3:** 세 method 모두 **15/15** 전 카테고리 재현 완료.
...
| hazelnut | 1.000 | 0.999 | 1.000 | PC=RD |
...
| **Mean (15개)** | **0.982** | **0.980** | **0.978** | **PC** |

## 4. 핵심 발견

### 4.1 지표별 우열 (15개 전 카테고리 기준)
- **탐지(I-AUROC):** SimpleNet(0.995) > PatchCore(0.992) > RD(0.988). 학습 기반 두 method가 이미지 단위 탐지에서 강하고, RD가 근소하게 뒤짐.
- **위치추정(Full P-AUROC):** PatchCore(0.982) > SimpleNet(0.980) = RD(0.978). memory bank의 세밀한 공간 정보가 픽셀 정밀도에서 여전히 1위. SimpleNet과 RD는 평균은 비슷하지만 **카테고리별 강점이 정반대**(아래 4.2~4.3).
...
- 15/15 전 카테고리 재현 완료를 바탕으로, 세 method의 특성을 통합한 4-way 비교 프레임워크 설계에 착수한다.
