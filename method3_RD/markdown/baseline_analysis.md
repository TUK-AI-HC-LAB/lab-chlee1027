# Reverse Distillation Baseline 재현 분석 (MVTec AD 15개 전 카테고리 완결)

> 재현 실행: 2026-05-21 ~ 2026-05-22, Colab T4 GPU
...
## 1. 개요 및 목적
Reverse Distillation(RD) baseline을 MVTec AD에서 재현하여, PatchCore(method1)·SimpleNet(method2)와 동일 조건에서 비교 가능한 세 번째 기준선을 확보한다. 15개 전 카테고리 재현 완료.
...
## 3. 종합 결과 (15개 전 카테고리)
- **I-AUROC 평균**: 0.988 (논문 0.985)
- **Full P-AUROC 평균**: 0.978 (논문 0.978)

원논문 Table 1·2의 "Ours(WResNet50, 256)" 값과 대조한 결과 **ΔI +0.003, ΔP +0.000으로 완벽히 일치** — 15개 전 카테고리 재현 성공. grid·hazelnut·leather·metal_nut·wood에서 I-AUROC 1.000을 달성했다.

> **논문 대조 완료:** `baseline_full_table.md`의 paper 컬럼을 원논문 PDF Table 1·2에서 직접 전사하여 정정함. 모든 카테고리가 Δ ±0.01 내로 정확히 재현됨.
...
## 5. 결론
RD baseline을 15개 전 카테고리에서 원논문과 거의 동일하게 재현(ΔI +0.003 / ΔP +0.000)하였으며, 세 method의 3-way 비교 기반을 완결했다. 15/15 재현 성공을 바탕으로 차세대 모델(Dinomaly 등) 도입을 준비한다.
