# Reverse Distillation Baseline 재현 분석 (MVTec AD 13/15 진행)
...
## 1. 개요 및 목적
Reverse Distillation(RD) baseline을 MVTec AD에서 재현하여, PatchCore(method1)·SimpleNet(method2)와 동일 조건에서 비교 가능한 세 번째 기준선을 확보한다. 현재 13/15 카테고리 완료(잔여: hazelnut, pill).
...
## 3. 종합 결과 (13/15 카테고리)
- **I-AUROC 평균**: 0.988
- **Full P-AUROC 평균**: 0.977

원논문 Table 1·2의 "Ours(WResNet50, 256)" 값(I 0.985 / P 0.977)과 대조한 결과 **ΔI +0.003, ΔP ±0.000으로 거의 완벽히 일치** — 재현 성공. grid·leather·metal_nut·wood에서 I-AUROC 1.000을 달성했다.
...
## 5. 결론
RD baseline을 13/15 카테고리에서 원논문과 거의 동일하게 재현(ΔI +0.003 / ΔP ±0.000)했으며, 세 method의 3-way 비교 기반을 마련했다. 잔여 hazelnut·pill 완료 후 15/15 평균을 확정하고, 특히 PatchCore가 가장 약했던 pill에서 RD 거동을 확인하는 것이 다음 과제다.
